...
if __name__ == "__main__":
    ...
    parser.add_argument('--radius', type=int, help='Gaussian deconvolve kernal radius')
    parser.add_argument('--sigma', type=int, help='Gaussian deconvolve kernal sigma')
    parser.set_defaults(root_power=0.6, sky_level_factor=0.6, hist_smoothing=601, sky_removal_passes=2, radius=7, sigma=2)
    ...
    if args.power_adjustment_output:
        logging.info(f"Saving pre-sky adjustment file {args.power_adjustment_output}")
    ...
    if args.sky_removal_output:
        logging.info(f"Saving pre-sky adjustment file {args.sky_removal_output}")
